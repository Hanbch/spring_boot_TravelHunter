...
<script src="/assets/js/ajax/pheed.js"></script><!-- 글상세보기 관련 비동기처리 -->
		...
		<div class="pheed_list">
			<ul>
				<c:forEach var="data" items="${boardList}">
					<li><a href="#" class="view_detail" id="${data.id}">
							<div class="photo">
								<ul>
			...
			for (var j = 0; j < like.length; j++) {
				if (n == j) {
					if (like[j].classList.contains("active") == false) {
						...
						likes(board_id);//비동기처리 호출
					} else {
						like[j].classList.remove("active");
						off(board_id);//비동기처리 호출
					}
